    ...
    ListView {
        ...
        onWrapModeChanged: {
            if (contentWidth === fullWidth) {
                contentWidth = Qt.binding(function() { return width - verticalScrollBar.width; });
            } else {
            ...
            TextArea {
                ...
                function updateSelection() {
                    ...
                    // If the item is inside the selection, and isn't any of the edges of it
                        ...
                        delegateElement.selectAll();

                    // If the item is the beginning and ending of the selection
                    } else if (index === s_index && index === e_index) {
                        delegateElement.select(selectionArea.start_position, selectionArea.end_position);

                    // If the selection starts at this item
                    } else if (index === s_index) {
                        delegateElement.select(selectionArea.start_position, delegateElement.length);

                    // If the selection ends at this item
                    } else if (index === e_index) {
                        delegateElement.select(0, selectionArea.end_position);
                    }
                ...
                // NOTE: This is done so that the delegate updates it's shown content after the property changed,
                //       otherwise it updates the layout, but visually it stays in the old state.
                ...
                Component.onCompleted: {
                    ...
                    const new_content_width = view.leftMargin + contentWidth + view.rightMargin + verticalScrollBar.width;
                    if (view.contentWidth < new_content_width) {
                        if (view.contentWidth === view.fullWidth || view.fullWidth === 0)
                            view.contentWidth = new_content_width;
                        view.fullWidth = new_content_width;
                    }
                }
